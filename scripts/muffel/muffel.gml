...
// same for nmframes
#macro nmframes       , "nmframes",  function(muffel_object_ref) { global.tmp_muffel_object_ref = muffel_object_ref; 
// and mframe
#macro mframe      -1, "mframe",   function(muffel_object_ref) { global.tmp_muffel_object_ref = muffel_object_ref; 
...
// if you want to access how many nmframes or seconds have passed
// (or how often your code was called), use these
#macro executed_nmframes  muffel_object_ref._passed_nmframes
...
function execute_every_n_nmframes(n, callback) {
...
function execute_after_n_nmframes(n, callback) {
	inst = execute_every_n_nmframes(n, callback)
	inst._abort_at_mframe = n;
}

function execute_every_mframe(callback) {
	execute_every_n_nmframes(1, callback);
}

function execute_every_second(callback) {
	execute_every_n_nmframes(room_speed, callback);
}

function check_n(n, type) {
	if(type == "seconds" || type == "nmframes") {
		if(n < 0) {
			show_error("You are trying to execute a block of code in negative number of seconds " +
			           "or nmframes (" + string(n) + "), but that can't be right " +
					   "(this script is awesome, but I can't time travel, you know)", true);
		} else if(n == 0) {
			show_error("You are trying to execute a block of code in zero seconds or nmframes, " +
			           "which doesn't make sense", true);
		}
	} else if (type == "second" || type == "mframe") {
...
function execute_every_n(n, type, callback) {
	...
	execute_every_n_nmframes(interval, callback);
}

function execute_after_n(n, type, callback) {
	var interval = muffel_execute_init(n, type);
	execute_after_n_nmframes(interval, callback);
}
